...
import inocilationGrowingService from '../../../../services/inocilationGrowingService';
import 'semantic-ui-css/semantic.min.css'
import TransferForm from './transferForm';
import { getLanguage } from '../../../global/language';

class TransferStock extends Form {
    constructor(props) {
        super(props);
        this.state = {
            ...
            options1: [
                ...
                {"type":"rafte", "value": 43, "text": `${getLanguage().raft} 43`},
                {"type":"rafte", "value": 44, "text": `${getLanguage().raft} 44`},
                {"type":"rafte", "value": 45, "text": `${getLanguage().raft} 45`},
                {"type":"rafte", "value": 46, "text": `${getLanguage().raft} 46`},
                {"type":"rafte", "value": 47, "text": `${getLanguage().raft} 47`}
            ],
            growing: 'choose',
            max_nr_blocks: false
        }
    ...
    schema = {
        ...
        nr_seed: Joi.number().integer().allow('').optional(),
    ...
    componentWillReceiveProps(nextProps)
    {
        ...
        let plant = nextProps.plant.inoculation_room;
        // let getDate = plant[plant.length - 1].date_seed
        // let day = getDate.split(' ')[0]
        ...
        // console.log(year)
        // console.log(date)
        // data.date_seeds = date
        let seed = plant[plant.length - 1].nr_seed
        data.nr_seed = seed
        this.setState({
            data
        })
    ...
    submitForm = () => {
        ...
        inocilationGrowingService.post(plant_id, nr_seed, nr_blocks_damage, date_seeds, note, histories).then(({ data }) => {
            this.setState({
                loading: false,
                data: {
                    plant_id: this.props.plant.id,
    ...
    onFridge = (element) => {
        inocilationGrowingService.postFridge(this.state.plant.id).then(({ data }) => {
        toast.success("Dergesa u be me sukses ne friz.");
        this.props.updatePlant();
        this.setState({
            growing: element
        })
    ...
    render() {
        let { options, options1, data, errors, plant, loading, growing, max_nr_blocks } = this.state;
        ...
        if(plant.inoculation_room_growing.length !== 0)
        {
            growing = 'growing'
        }
        return (
                    ...
                    <div className={`timeline-info ${pointer === 6 ? 'active' : ''}`} >
                        <div className={`timeline-click `}>
                            <span onClick={toggleList.bind(this)} className="title">
                                ...
                                {getLanguage().block_transfer}
                            </span>
                        </div>
                        {plant.status === 'Bartur ne Friz' && plant.inoculation_room_growing.length === 0 &&
                            <div className={`timeline-show fridge ${growing === 'fridge' ? '' : 'disable'}`}>
                                <form autoComplete="off" onSubmit={this.handleSubmit} className={`choose-transfer`} >
                                    <p>Bartur ne friz</p>
                                </form>
                            </div>
                        }
                        {plant.status === 'Dhoma Inokulimit' &&
                            <div className={`timeline-show buttons ${growing === 'choose' ? '' : 'disable'}`}>
                                <form autoComplete="off" onSubmit={this.handleSubmit} className={`choose-transfer`} >
                                    <span onClick={this.onGrowing.bind(this, 'growing')} className={`choose-button`}>{getLanguage().rising}</span>
                                    <span onClick={this.onFridge.bind(this, 'fridge')} className={`choose-button`}>{getLanguage().fridge}</span>
                                </form>
                            </div>
                        }
                        {plant.inoculation_room_growing.map((element, index) => (
                            <div key={index} className={`timeline-show ${pointer === 6 || growing === 'growing' ? 'active' : 'display-none'}`}>
                                                ...
                                                element.shelf.map((shelf, idx) =>
                                                    <div key={idx} className={`extra-form map`}>
                                                        <div className="materials">
                                                            <span>Numri Seres: {shelf.green_house_growing_id}</span>
                                                        </div>
                                                        <div className="materials">
                        ...
                        {plant.status !== 'Bartur ne Friz' && (role === 'admin' || (privileges.access_inoculation_room && plant.inoculation_room_growing && plant.inoculation_room_growing.length < 1)) &&
                            <div className={`timeline-show ${pointer === 6 || growing === 'growing' ? 'active' : 'display-none'}`}>
                                <div className={`timline-helper`}>
                                    <div className={`left-timeline`}>
                                        <img src="./assets/img/circle.svg" alt="" />
                                        <div className="line">
                                    ...
                                    <div className={`right-timeline`}>
                                        ...
                                        <div className={`progress-content`}>
                                            <form autoComplete="off" onSubmit={this.handleSubmit} >
                                                <div className="materials">
                                                    {this.renderInput("nr_seed", null, 'nr_seed', nr_seed, this.handleChange, errors.nr_seed, false, `${getLanguage().nr_seed}`, true, 'number')}
                                                </div>
                                                <div className="materials">
